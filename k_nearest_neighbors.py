...
class KNearestNeighbors:
    ...
    def predict(self, X):
        ...
        for i in test_data:
            dist_array = []
            for j in train_data:
                dist_array.append(self._distance(i,j))
            # print(dist_array)
            # print(self._y)
            # print(dist_array)
            final_array  = [y for x, y in sorted(zip(dist_array, train_y))]
            final_array = final_array[0:self.n_neighbors]
            # print(final_array)
            seg_value = max(final_array, key=final_array.count)
            
            # print(seg_value)
            result_list.append(seg_value)
        # print(result_list)
        return result_list
        # raise NotImplementedError('This function must be implemented by the student.')
